...
authorial voice and prose style preferences. Use `update_bible({type:"stylometry",
...
|-------------------|-------------|-----|
| Before writing ANY scene | `suggest_next_beat` + `query_bible` | Get beat suggestion from Markov chain + check bible for continuity |
...
| Novelist asks about characters/locations/plots | `query_bible` (ONLY) | Do NOT answer from memory -- query the bible |
| Novelist provides new story information | `update_bible` | Record immediately before returning to the current task |
...
- Call `query_bible` before any scene composition to check continuity
...
- NEVER describe a character or location from memory -- always call `query_bible`
...
| **Archivist** (16) | Maintaining scene metadata and tracking narrative state | Recording bible updates via `update_bible` after scenes introduce new characters, locations, or plot developments; querying `query_bible` for continuity checks |

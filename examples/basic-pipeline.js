...
import { 
// Data model / bible
InMemoryStoryBible, PlotThreadTracker, DramaticQuestionTracker, ArcTrackerImpl, ThematicProgressionImpl, SceneStatus, writeScene, readScene, serializeScene, parseScene, 
// Mailbox
Mailbox, 
// Agent system
AgentRole, BaseAgent, AgentRegistry, 
// Markov engine
MarkovInferenceEngine, MarkovPathPredictor, 
// Context
CheckpointManager, SlidingWindowManager, ContextWindowBudget, TieredMemory, CondensationStrategy, 
// Pipeline
PipelineOrchestrator, PipelinePhase, InterviewProtocol, ChapterDeductor, BibleUpdateProtocol, IterationBudget, defaultBudgetConfig, ConflictResolution, ScenePlanGate, SceneProseGate, 
// Wiki
parseWikiPageContent, WikiLinkExtractor, WikiPageType, WikiToBiblePopulation, } from "@kleptowriter/kleptowriter-core";
...
class WriterAda extends BaseAgent {
    constructor() { super({ id: "writer-ada", role: AgentRole.Writer, capabilityTier: ["prose-gen", "creativity"], mode: "generation" }); }
}
class CriticBen extends BaseAgent {
    constructor() { super({ id: "critic-ben", role: AgentRole.Critic, capabilityTier: ["evaluation", "analysis"], mode: "evaluation" }); }
}
class ResearcherEve extends BaseAgent {
    constructor() { super({ id: "researcher-eve", role: AgentRole.Researcher, capabilityTier: ["research", "analysis"], mode: "research" }); }
}
class FactCheckerOscar extends BaseAgent {
    constructor() { super({ id: "factchecker-oscar", role: AgentRole.FactChecker, capabilityTier: ["fact-checking", "analysis"], mode: "evaluation" }); }
}
...
// Mailbox messaging
const msg = mailbox.deliver({ agentId: "critic-ben" }, { type: "evaluation", from: { agentId: "writer-ada" }, to: { agentId: "critic-ben" }, payload: { sceneId: "scene-001" } });
...
const diffResult = new MetadataDiff().diff(scene1.metadata, scene2.metadata);
...
const fitted = ctxBudget.fitWithinBudget([
    { id: "s1", content: scene1.prose, priority: 10, estimatedTokens: 100, category: "scene" },
    { id: "sum", content: "Summary", priority: 8, estimatedTokens: 30, category: "summary" },
]);
...
if (wpr.ok)
    console.log("  Wiki page:", wpr.data.name, "| type:", wpr.data.type === WikiPageType.Character ? "Character" : "?");
...
if (wpr.ok)
    console.log("  Entities created:", pop.populate([wpr.data], bible).entitiesCreated);
// ═════════════════════════════════════════════════════════════════════
//  ✅  DONE
// ═════════════════════════════════════════════════════════════════════
console.log("\n═══ ✅ Demo Complete ═══");
